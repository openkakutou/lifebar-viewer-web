...
- `elements/element-layout.ts` is pure logic (no DOM): computes where each recognized section draws, from its own raw `pos`/`N.spr`/`N.offset` entries, and resolves each `N.spr` layer against the loaded sprite sheet's metadata into one of three states (no sheet yet, invalid reference, resolved) — see `.vibe/decisions/004-element-layout-convention-and-placeholder-states.md`. A section whose entries carry a `p1.`/`p2.` key prefix (a shared Ikemen GO section holding both players' data at once) is split by `computeSectionElementLayouts` into two `ElementLayout`s, one per player, computed from only that player's own entries with the prefix stripped; a section mixing prefixed and unprefixed layout-relevant entries, or with only one player's prefix present, becomes a single element flagged with an `issue` instead of a guessed layout — see `.vibe/decisions/009-shared-section-per-player-split-and-malformed-state.md`.
...
1. Every recognized section becomes one or two `ElementLayout`s via `element-layout.ts`'s `computeSectionElementLayouts`: a classic section (no `p1.`/`p2.` prefix) becomes one, exactly as `computeElementLayout` alone always produced — its `pos` entry as an origin point, plus one `ElementLayer` per `N.spr` entry (offset by the matching `N.offset`, `(0, 0)` otherwise); a shared section holding both players' data becomes two, one per player, from that player's own prefixed entries; a section with inconsistent player prefixing becomes one, flagged with an `issue` and no sprite layers, anchored at a fixed neutral origin instead of a guessed position — see `.vibe/decisions/009-shared-section-per-player-split-and-malformed-state.md`.
...
3. `elements-panel.ts` flattens every section's one or two `ElementLayout`s into the list it renders, remembering each one's originating section index (so, e.g., a simulated value still looks up the right slot after a split). It renders one list button and one absolutely-positioned overlay `<div>` per element from that box — the overlay, not the canvas, carries the `is-selected` highlight and the `no-sprite`/`unresolved`/`waiting`/`malformed-prefix` state classes, since a canvas redraw would need to re-blit everything just to toggle a highlight.
