...
// everything else `any` too -- the same approach the sibling `lifebar-editor`
// repo already took for the same missing-declarations gap (its own
// `src/types/web-ui-kit.d.ts`, for `CommandStack`/`ShortcutManager`). The
...
// confirmed by bisection, cause not otherwise diagnosed -- so this app
// avoids that combination entirely rather than depending on unconfirmed
// compiler behavior.
declare module "@openkakutou/web-ui-kit" {
   ...
   * to this type explicitly instead, the same "as unknown as" shape
   * `lifebar-editor`'s own shim already uses for `WuikShortcutsPanelElement`. */
