...
- [ ] If a `wasm:download`-equivalent script exists for `sff` by the time this item is picked up, add it as a build step before `npm run build`, mirroring `character-viewer-web`'s equivalent step; if not, the workflow still deploys whatever the app currently is — nothing here should wait for this repo's own item `002`/`003` to finish

## Notes
No hard dependency on other backlog items — this is infrastructure, not a feature. Deploying now, ahead of parsing/rendering landing, is deliberate: every later feature then lands live automatically on its own next merge.
