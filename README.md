...
- Load a lifebar by selecting or dropping the folder it's distributed in — both classic MUGEN and modern Ikemen GO lifebar files are supported, including Ikemen GO's simultaneous/turns/tag mode variants. If the folder has exactly one lifebar file it loads automatically; if it has several, you're asked which one to load. A malformed file shows a clear error, and any section this app doesn't yet recognize is skipped with a warning instead of blocking the load.
